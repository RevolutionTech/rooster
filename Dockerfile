...
WORKDIR /usr/
RUN virtualenv -p /var/lang/bin/python compile-sqlite
RUN git clone https://github.com/coleifer/pysqlite3
WORKDIR /usr/pysqlite3/
RUN cp /usr/sqlite-33001/sqlite3.[ch] .
RUN source /usr/compile-sqlite/bin/activate && python setup.py build_static && python setup.py install

WORKDIR /var/task

RUN cp /usr/compile-sqlite/lib/python3.8/site-packages/pysqlite3-0.4.4-py3.8-linux-x86_64.egg/pysqlite3/_sqlite3.cpython-38-x86_64-linux-gnu.so .
